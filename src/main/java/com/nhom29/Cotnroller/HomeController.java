...
//        ========================================================
        ...
        double tongTrang = tagInter.getAllTag().size() / (soLuongPhanTu*1.0);
        Integer pageTotal = (int)Math.ceil(tongTrang);

        model.addAttribute("tagList", tagInter.phanTrangTag(page - 1, soLuongPhanTu, sort));

        Pagination pagination = new Pagination(pageTotal, page);
        model.addAttribute("pagination", pagination);
